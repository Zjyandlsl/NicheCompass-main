...
class NicheCompass(BaseModelMixin):
    ...
    #     spatial_coords_key: Optional[str] = "spatial",
    #     n_neighbors: int = 6,
    #     **kwargs,
    # ) -> None:
    #     """
    #     标准化 AnnData 对象，为 NicheCompass 训练做准备。
    #     （此处保留之前提供的方法注释和完整逻辑）
    #     """
    #     # 【粘贴之前提供的setup_anndata完整逻辑】
    #     # 1. 校验表达矩阵
    #     if counts_key is not None:
    #         if counts_key not in adata.layers:
    #             raise ValueError(f"原始计数未在 adata.layers['{counts_key}'] 中找到！")
    #         counts = adata.layers[counts_key]
    #         if sp.issparse(counts):
    #             counts = counts.toarray()
    #         if (counts < 0).any() or not np.issubdtype(counts.dtype, np.integer):
    #             raise ValueError("adata.layers['{counts_key}'] 必须是原始整数计数！")
    #     # 2. 构建/校验空间邻接矩阵（完整逻辑）
    #     # 3. 校验GP掩码（完整逻辑）
    #     # 4. 计算基因/peak索引（完整逻辑）
    #     # 5. ATAC数据校验（完整逻辑）
