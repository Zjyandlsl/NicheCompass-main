...
# 源文件：/home/zhangjunyi/xiangmu/nichecompass-main/TuMeNiche/src/my/case/parameter_analysis/n_epochs_lr/SN048_A121573_Rep1_n_epochs_AND_lr_heatmap.ipynb
...
# 0. 当前数据集与参数分析配置
# --------------------------
CONFIG = {'dataset_name': 'CRC_SN048_A121573_Rep1',
 'input_h5ad': '/home/zhangjunyi/xiangmu/nichecompass-main/datasets/12_colorectal_cancer_CRC/12_colorectal_cancer_CRC_h5ad/SN048_A121573_Rep1.h5ad',
 'gmm_threshold_csv_candidates': ['/home/zhangjunyi/xiangmu/nichecompass-main/outputs/test/colorectal_12_cancer_CRC/SN048_A121573_Rep1/SpaCET/GMM_Optimal_Threshold_Result.csv',
                                  '/home/zhangjunyi/xiangmu/nichecompass-main/outputs/colorectal_12_cancer_CRC/SN048_A121573_Rep1/SpaCET/GMM_Optimal_Threshold_Result.csv'],
 'save_root': '/home/zhangjunyi/xiangmu/nichecompass-main/outputs/case/parameter_analysis/n_epochs_lr/SN048_A121573_Rep1',
...
# 断点续跑版：已存在 .h5ad 的组合自动跳过，只补跑缺失组合
# =========================================================
...
def find_existing_h5ad(run_dir):
    """在某个参数组合文件夹中查找已经保存的 h5ad 文件。"""
    ...
    return os.path.join(run_dir, h5ad_files[0])
...
def run_parameter_grid_analysis():
    ...
    for n_epochs in y_values:
        for lr in x_values:
            ...
            existing_h5ad = find_existing_h5ad(run_dir)

            if existing_h5ad is None:
                missing_jobs.append((n_epochs, lr))
            else:
                finished_jobs.append((n_epochs, lr, existing_h5ad))

    print("\n" + "=" * 90)
    print(f"📌 参数组合总数: {total_runs}")
    print(f"✅ 已完成组合数: {len(finished_jobs)}")
    print(f"❌ 待补跑组合数: {len(missing_jobs)}")

    if len(missing_jobs) > 0:
        print("待补跑组合:")
        for n_epochs, lr in missing_jobs:
            print(f" - n_epochs={n_epochs}, lr={lr}")

    print("=" * 90)

    # =====================================================
    # 1. 只有存在缺失组合时，才重新计算 Sender / Receiver 分数
    # =====================================================
    hard_tumor_threshold = None
    adata_scored = None

    if len(missing_jobs) > 0:
        print("\n🔄 检测到存在未完成组合，开始准备补跑所需数据...")
        hard_tumor_threshold = read_gmm_threshold()
        adata_scored = compute_sender_receiver_scores()
    else:
        print("\n✅ 所有组合已有 .h5ad，不重新计算 Sender / Receiver，不重新训练模型。")

    # =====================================================
    # 2. 主循环：已完成则读取 h5ad 算 ARI；未完成则训练补跑
    # =====================================================
    run_idx = 0
    ...
    for n_epochs in y_values:
        for lr in x_values:
            ...
            # =================================================
            # A. 已完成：跳过训练，直接读取已有 h5ad 计算 ARI
            # =================================================
            if existing_h5ad is not None:
                print(f"⏭️ 已完成，跳过训练: {existing_h5ad}")

                try:
                    adata_done = sc.read_h5ad(existing_h5ad)

                    ari, n_valid, pathology_col = calculate_level1_ari(adata_done)
                    ...
                    print(f"⚠️ 已有 h5ad 读取或计算 ARI 失败，将重新补跑该组合。")
                    print(f"⚠️ 问题文件: {existing_h5ad}")
                    print(f"⚠️ 错误信息: {e}")

                    existing_h5ad = None

                    if hard_tumor_threshold is None or adata_scored is None:
                        print("🔄 为补跑该组合，开始重新准备数据...")
                        hard_tumor_threshold = read_gmm_threshold()
                        adata_scored = compute_sender_receiver_scores()
